...
pub fn ncbiannotate(pathncbi: &str, clinvar: &str, medgenomim: &str, medgenhpo: &str, omimsearch: &str) -> Result<Vec<NCBIgeneMerged>, Box<dyn Error>>{
      ...
      let clinvarmapwrite:Vec<ClinVarOMIM> = clinvarmapper(clinvar, medgenomim, medgenhpo, omimsearch).unwrap();
...
pub fn mapiter(lineread: String) -> std::io::Result<Vec<NCBIgene>> {
    ...
    if !line.starts_with("#") {
        ...
        ncbigene.push(NCBIgene {
            ...
            disease: linesplit[3].to_string().split(":").collect::<Vec<_>>()[1].parse::<usize>().unwrap(),
        });
    }
    Ok(ncbigene)
}
